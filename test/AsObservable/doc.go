...
interface{} items of the source to bar items.

If the assertion is invalid, this will emit a type assertion error
ErrTypecastToBar at run-time. When AsObservableBar is called on an
ObservableFoo, then a type conversion is generated. This has to be possible,
